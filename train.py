...
from utils.model import DNAdescriminator, DNAgenerator
...
def train_gan(epoch_number, train_loader, device, gradient_clipping=False):
    ...
    for epoch in range(epoch_number):
        for idx, batch in enumerate(train_loader):
            ...
            for i in range(5):
                descr_loss = descriminator_part(descriminator, generator, input,\
                    z_size, desc_loss_list, grad_penalty_list, label, \
            ...
            gen_loss = generator_part(descriminator, generator, input, z_size,\
                 gen_loss_list, batch_diversity_list, label)    
            gen_optim.zero_grad()
            gen_loss.backward()
            ...
            if idx % 100 == 0:
                ...
                formated_string = "{epoch} D:{descr:1.3f}, D_grad:{grad:1.3f}, G_diver:{diver:1.3f} G:{gen:1.3f}" \
                    .format(epoch=epoch, descr=descr_loss, grad=gradient_penalty, \
                        diver=batch_diversity, gen=gen_loss)
        ...
        end_of_gan_epoch(generator, descriminator, input,\
             z_size, label, path_to_save=None)
